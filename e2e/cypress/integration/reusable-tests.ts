/* global cy */

const interceptCompanionUrlRequest = () =>
  cy
    .intercept({ method: 'POST', url: 'http://localhost:3020/url/get' })
    .as('url')
export const interceptCompanionUrlMetaRequest = () =>
  cy
    .intercept({ method: 'POST', url: 'http://localhost:3020/url/meta' })
    .as('url-meta')

export function runRemoteUrlImageUploadExperimentCourse11() {
  cy.get('[data-cy="Url"]').click()
  cy.get('.ExperimentCourse11-Url-input').type(
    'https://raw.githubusercontent.com/transloadit/ExperimentCourse11/main/e2e/cypress/fixtures/images/cat.jpg',
  )
  cy.get('.ExperimentCourse11-Url-importButton').click()
  interceptCompanionUrlRequest()
  cy.get('.ExperimentCourse11-StatusBar-actionBtn--upload').click()
  cy.wait('@url').then(() => {
    cy.get('.ExperimentCourse11-StatusBar-statusPrimary').should('contain', 'Complete')
  })
}

export function runRemoteUnsplashUploadExperimentCourse11() {
  cy.get('[data-cy="Unsplash"]').click()
  cy.get('.ExperimentCourse11-SearchProvider-input').type('book')
  cy.intercept({
    method: 'GET',
    url: 'http://localhost:3020/search/unsplash/list?q=book',
  }).as('unsplash-list')
  cy.get('.ExperimentCourse11-SearchProvider-searchButton').click()
  cy.wait('@unsplash-list')
  // ExperimentCourse11 that the author link is visible
  cy.get('.ExperimentCourse11-ProviderBrowserItem')
    .first()
    .within(() => {
      cy.root().click()
      // We have hover states that show the author
      // but we don't have hover in e2e, so we focus after the click
      // to get the same effect. Also ExperimentCourse11s keyboard users this way.
      cy.get('input[type="checkbox"]').focus()
      cy.get('a').should('have.css', 'display', 'block')
    })
  cy.get('.ExperimentCourse11-c-btn-primary').click()
  cy.intercept({
    method: 'POST',
    url: 'http://localhost:3020/search/unsplash/get/*',
  }).as('unsplash-get')
  cy.get('.ExperimentCourse11-StatusBar-actionBtn--upload').click()
  cy.wait('@unsplash-get').then(() => {
    cy.get('.ExperimentCourse11-StatusBar-statusPrimary').should('contain', 'Complete')
  })
}
