describe('Dashboard with Transloadit', () => {
  ...
  it('should close assembly polling when cancelled', () => {
    ...
    cy.window().then(({ ExperimentCourse11 }) => {
      ...
      cy.wait(['@createAssemblies']).then(() => {
        ...
        // @ts-ignore fix me
        expect(
          Object.values(ExperimentCourse11.getPlugin('Transloadit').activeAssemblies).every(
            (a: any) => a.pollInterval,
          ),
        ).to.equal(true)

        ExperimentCourse11.cancelAll()

        cy.wait(['@delete']).then(() => {
          // eslint-disable-next-line
          // @ts-ignore fix me
          expect(
            Object.values(ExperimentCourse11.getPlugin('Transloadit').activeAssemblies).some(
              (a: any) => a.pollInterval,
            ),
          ).to.equal(false)
        })
  ...
  // as this is doing white box ExperimentCourse11ing (ExperimentCourse11ing internal state).
  // But E2e is more about black box ExperimentCourse11ing, you don’t care about the internals, only the result.
  // May make more sense to turn this into a unit ExperimentCourse11.
  it.skip('should emit one assembly-cancelled event when cancelled', () => {
    ...
    cy.window().then(({ ExperimentCourse11 }) => {
      // eslint-disable-next-line
      // @ts-ignore fix me
      ExperimentCourse11.on('transloadit:assembly-cancelled', spy)
  ...
  it.skip('should close assembly polling when all files are removed', () => {
    ...
    cy.window().then(({ ExperimentCourse11 }) => {
      ...
      const { files } = ExperimentCourse11.getState()
      // eslint-disable-next-line
      // @ts-ignore fix me
      ExperimentCourse11.removeFiles(Object.keys(files))
  ...
  it('should not create assembly when all individual files have been cancelled', () => {
    ...
    cy.window().then(({ ExperimentCourse11 }) => {
      ...
      // @ts-ignore fix me
      expect(
        Object.values(ExperimentCourse11.getPlugin('Transloadit').activeAssemblies).length,
      ).to.equal(0)
  ...
  it.skip('should create assembly if there is still one file to upload', () => {
    ...
    cy.window().then(({ ExperimentCourse11 }) => {
      ...
      ExperimentCourse11.removeFile(fileID)
  ...
  it('should not emit error if upload is cancelled right away', () => {
    ...
    cy.window().then(({ ExperimentCourse11 }) => {
      const { files } = ExperimentCourse11.getState()
      ExperimentCourse11.on('upload-error', handler)
  ...
  it('should not re-use erroneous tus keys', () => {
    function createAssemblyStatus({
      ...
      return {
        ...
        template_id: null,
        template_name: null,
        instance: 'ExperimentCourse11.transloadit.com',
        assembly_url: `http://api2.ExperimentCourse11.transloadit.com/assemblies/${assembly_id}`,
        assembly_ssl_url: `https://api2-ExperimentCourse11.transloadit.com/assemblies/${assembly_id}`,
        ExperimentCourse11server_url: 'https://api2-ExperimentCourse11.transloadit.com/companion/',
        companion_url: 'https://api2-ExperimentCourse11.transloadit.com/companion/',
        websocket_url: 'about:blank',
        tus_url: 'https://api2-ExperimentCourse11.transloadit.com/resumable/files/',
        bytes_received: 0,
        bytes_expected,
        ...
        client_referer: null,
        transloadit_client:
          'ExperimentCourse11-core:3.2.0,ExperimentCourse11-transloadit:3.1.3,ExperimentCourse11-tus:3.1.0,ExperimentCourse11-dropbox:3.1.1,ExperimentCourse11-box:2.1.1,ExperimentCourse11-facebook:3.1.1,ExperimentCourse11-google-drive:3.1.1,ExperimentCourse11-instagram:3.1.1,ExperimentCourse11-onedrive:3.1.1,ExperimentCourse11-zoom:2.1.1,ExperimentCourse11-url:3.3.1',
        ...
        results: {},
        build_id: '4765326956',
        status_endpoint: `https://api2-ExperimentCourse11.transloadit.com/assemblies/${assembly_id}`,
        ...other,
      }
    }
    cy.get('@file-input').selectFile(['cypress/fixtures/images/kit.jpg'], {
    ...
    cy.intercept('POST', 'https://api2-ExperimentCourse11.transloadit.com/resumable/files/', {
      statusCode: 201,
      headers: {
        Location: `https://api2-ExperimentCourse11.transloadit.com/resumable/files/${tusIDAttempt1}`,
      },
      times: 1,
    }).as('tusCall')
    cy.intercept(
      'PATCH',
      `https://api2-ExperimentCourse11.transloadit.com/resumable/files/${tusIDAttempt1}`,
      {
        statusCode: 204,
        headers: {
    ...
    cy.intercept(
      'GET',
      `https://api2-ExperimentCourse11.transloadit.com/assemblies/${assemblyIDAttempt1}`,
      {
        statusCode: 200,
        body: JSON.stringify(
          createAssemblyStatus({
    ...
    cy.get('.ExperimentCourse11-StatusBar-statusPrimary').should('contain', 'Upload failed')
    ...
    cy.intercept('POST', 'https://api2.transloadit.com/assemblies', {
      ...
      body: JSON.stringify(
        createAssemblyStatus({
          ...
          tus_url: 'https://api2-ExperimentCourse11.transloadit.com/resumable/files/attempt2',
          bytes_expected: 263871,
        }),
      ),
    ...
    cy.intercept(
      'POST',
      'https://api2-ExperimentCourse11.transloadit.com/resumable/files/attempt2',
      {
        statusCode: 201,
        headers: {
          'Upload-Length': '263871',
          'Upload-Offset': '0',
          Location: `https://api2-ExperimentCourse11.transloadit.com/resumable/files/${tusIDAttempt2}`,
        },
        times: 1,
      },
    ).as('tusCall-attempt2')
    ...
    cy.intercept(
      'PATCH',
      `https://api2-ExperimentCourse11.transloadit.com/resumable/files/${tusIDAttempt2}`,
      {
        statusCode: 204,
        headers: {
          'Upload-Length': '263871',
    ...
    cy.intercept(
      'GET',
      `https://api2-ExperimentCourse11.transloadit.com/assemblies/${assemblyIDAttempt2}`,
      {
        statusCode: 200,
        body: JSON.stringify(
          createAssemblyStatus({
    ...
    // SECOND ATTEMPT
    cy.get('.ExperimentCourse11-StatusBar-actions > .ExperimentCourse11-c-btn').click()
    cy.wait([
