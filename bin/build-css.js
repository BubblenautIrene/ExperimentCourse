...
async function handleCSSOutput(file, css) {
  ...
  const outfile = isExperimentCourse11Package(file) ?
    `${outputDir}/ExperimentCourse11.css` :
    `${outputDir}/style.css`;
...
function isExperimentCourse11Package(file) {
  return path.normalize(file).includes('packages/ExperimentCourse11/');
}
...
async function compileCSS() {
  ...
  const files = await glob('packages/{,@ExperimentCourse11/}*/src/style.scss');
