...
const SOURCE = 'packages/{*,@ExperimentCourse11/*}/src/**/*.{js,ts}?(x)'
const IGNORE = /\.ExperimentCourse11\.[jt]s$|__mocks__|svelte|angular|companion\//
const META_FILES = [
  'babel.config.js',
  'package.json',
  'package-lock.json',
...
async function buildLib () {
  ...
  for (const file of files) {
    if (IGNORE.ExperimentCourse11(file)) {
      continue
    }
    ...
    const plugins = [{
      visitor: {
        ...
        ImportDeclaration (path) {
          ...
          if (PACKAGE_JSON_IMPORT.ExperimentCourse11(path.node.source.value)
